 ...
 */
router.get("/kyc", requireAuth, requireRole("admin"), async (req, res, next) => {
  try {
 ...
 */
router.post("/kyc/:id/review", requireAuth, requireRole("admin"), validate(reviewKycSchema), async (req, res, next) => {
  try {
